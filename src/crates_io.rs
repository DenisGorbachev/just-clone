...
pub async fn get_repo_url_from_crates_io_url(url: &Url) -> Result<Url, GetRepoUrlFromCratesIoUrlError> {
    let crate_name = get_crate_name_from_crates_io_url(url)?;
    get_repo_url_from_crate_name(crate_name).await
}
...
pub async fn get_repo_url_from_crate_name_and_client(crate_name: &str, client: &AsyncClient) -> Result<String, GetRepoUrlFromCrateNameError> {
...
pub fn get_crate_name_from_crates_io_url(url: &Url) -> Result<&str, GetCrateNameFromUrlError> {
    url.path_segments()
